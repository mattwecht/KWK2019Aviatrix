...
func fly(myPlane : Aviatrix) {
    ...
    for (index, city) in destinations.enumerated() {
        let distance = myPlane.distanceTo(currentLocation: myPlane.location, target: city)
...
func refuel(myPlane : Aviatrix) {
    ...
    let cost = myPlane.data.fuelPrices[myPlane.location]!
    
    print("Refueling...")
    print("⛽ Here in \(myPlane.location), jet fuel costs $\(cost)")
    print("⛽ You refueled \(refuelData) gallons totaling \(cost*refuelData)")
    myPlane.fuelCost += cost*refuelData
}

func fuelCheck(myPlane : Aviatrix, destination : String) -> Bool {
    let distanceToTravel =  Double(myPlane.distanceTo(currentLocation: myPlane.location, target : destination))
...
var plane = Aviatrix(author: "Sherly")
...
print(" ")
print("\(plane.author) you cant be serious, dont leave!")
